...
[English](https://github.com/B-2U/ISAC/blob/main/README.md) | [中文](https://github.com/B-2U/ISAC/blob/main/README_zh.md) | **日本語**
...
| 指定方法           | 入力例                         |
| ------------------ | ------------------------------ |
| `[region] <ign>`   | `.wws asia B2U` または `.wws B2U` |
| `<@mention>`       | `.wws @B2U#0900`               |
| `<discordUserID>`  | `.wws 930855839961591849`      |
| `me`               | `.wws me`                      |
| 画像を添付         | プレイヤーのIGNが写ったゲーム内スクリーンショットを添付 |
...
| `.top`<br>`.dalao`         | `.top [region] <shipName>`                        | `.top Yamato` <br> `.top NA Slava`                                                 | トップ15のプレイヤーを調べることができます <br> from [Kokomi API](https://github.com/SangonomiyaKoko/Kokomi_Backend)                                                                                                   |
...
`/wws`、`/recent`、`.wws`、`.recent`、`.uid`、`.history`にゲーム内スクリーンショットを添付すると、IGNを入力せずにプレイヤーを検索できます。OCRはサーバーのデフォルトのWorld of Warships地域を使用します。prefixコマンドに地域を入力していても同じです。

画像が添付されている場合は画像のOCR結果を優先し、入力されたプレイヤー引数は無視します。画像は2 MiB以下、幅と高さは4096ピクセル以下で、IGNが1つだけはっきり読める必要があります。OCRが認識できるのはASCII英字、数字、`_`、`[`、`]`で、長さは3～24文字です。OCRモデルは初回起動時にダウンロードされ、`user_data/ocr`にキャッシュされます。
